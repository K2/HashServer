...
namespace Reloc
{
    // stupid checksum needs the whole file!
    public class ReReDB
    {
        ...
        public DeLocate GetLocated(bool Is64, string NormalizedName, uint TimeStamp, ulong CurrVA)
        {
            ...
            var deLoc = new DeLocate(OrigImageBase, DeLocate.ProcessRelocs(null, reBytes));
    ...
    {
        public DeLocate(ulong imageBase, RelocSection rs)
        {
        ...
        public async Task<string> DeLocateFile(string fPath, string RelocFile, ulong CurrBase, string SaveTo, bool is64 = false, bool FixHeader = false, bool ScaleFileAlignment = false)
        {
            ...
            var prepared = ProcessRelocs(null, relocData).ToArray();
            ...
            using (var fsRelocted = new FileStream(fPath, FileMode.Open, FileAccess.Read, FileShare.Read, PAGE_SIZE, true))
            {
                ...
                using (var fsWriteOut = new FileStream(SaveTo, FileMode.Create, FileAccess.Write, FileShare.Write, PAGE_SIZE, true))
                {
                    ...
                    for (uint i = 0; i < Chunks; i++)
                    {
                        ...
                        if (i == 0 && FixHeader)
                            DelocateHeader(readBuffer, OrigImageBase, (int)hdrFix.ImageBaseOffset, hdrFix.Is64);
        ...
        public RelocSection RelocData { get; }
        ...
        /// <returns></returns>
        public static List<Reloc> ProcessRelocs(RelocSection rs = null, byte[] inBuff = null)
        {
            ...
            if (rs != null)
            {
                if (rs.RawRelocBuffer != null)
                {
                    FileBuff = rs.RawRelocBuffer;
                    count = rs.RelocLength;
                }
                else
                {
                    FileBuff = File.ReadAllBytes(rs.FullPath);
                    count = FileBuff.Length;
                }
            }
            else if (inBuff != null)
            {
            ...
            using (var ms = new MemoryStream(FileBuff, index, count))
        ...
        public static void DelocateHeader(byte[] bytes, ulong OrigBase, int OrigBaseOffset, bool Is64)
        {
            byte[] valueBytres = null;

            if (Is64)
                valueBytres = BitConverter.GetBytes(OrigBase);
            else
            {
                var uintBase = (uint)OrigBase;
                valueBytres = BitConverter.GetBytes(uintBase);
            }

            for (int i = OrigBaseOffset, j = 0; i < OrigBaseOffset + (Is64 ? 8 : 4); i++)
            {
                bytes[i] = valueBytres[j++];
            }
        }
